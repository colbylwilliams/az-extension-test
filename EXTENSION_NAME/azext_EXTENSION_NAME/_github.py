...
def get_github_releases(org=EXT_REPO_OWNER, repo=EXT_REPO_NAME, prerelease=False):
...
def get_github_release(org=EXT_REPO_OWNER, repo=EXT_REPO_NAME, version: str = None, prerelease=False):
...
def get_github_latest_release_version(org=EXT_REPO_OWNER, repo=EXT_REPO_NAME, prerelease=False) -> str:
    log.info(f'Getting latest release version from GitHub ({org}/{repo})')
    version_json = get_github_release(org, repo, prerelease=prerelease)
    return version_json['tag_name']


def github_release_version_exists(version: str, org=EXT_REPO_OWNER, repo=EXT_REPO_NAME):
    log.info(f'Checking if release version {version} exists on GitHub ({org}/{repo})')
...
def get_release_asset(asset_url: str, to_json=True):  # pylint: disable=inconsistent-return-statements
...
def get_release_templates(version: str = None, prerelease=False, templates_url: str = None):
    if templates_url is None:
        version = version or get_github_latest_release_version(prerelease=prerelease)
        templates_url = f'https://github.com/{EXT_REPO_OWNER}/{EXT_REPO_NAME}/releases/download/{version}/templates.json'
    log.info(f'Getting templates.json from GitHub release ({version})')
...
def get_template_url(templates, parent: str, name: str) -> str:
    log.info(f'Getting url from template.json for {parent}.{name}')
